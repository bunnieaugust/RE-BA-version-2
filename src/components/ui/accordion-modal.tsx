      ...
      {items.slice(0, 11).map((item, i) => {
        return (
          <motion.img
            ...
            whileTap={{ scale: 0.95 }}
            className={cn(
              'rounded-2xl h-[200px] shrink-0 object-cover transition-[width] ease-in-out duration-300 cursor-pointer',
              index === i
                ? 'w-[250px]'
...
export default function AccordionModal({ items = itemsArr }: { items?: typeof itemsArr }) {
      ...
      <AnimatePresence>
        {open !== false && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            key='overlay'
            className='bg-brand-charcoal/60 dark:bg-black/80 backdrop-blur-xl fixed inset-0 z-50 top-0 left-0 bottom-0 right-0 w-full h-full grid place-content-center'
            onClick={() => {
              setOpen(false);
            }}
          >
            <div onClick={(e) => e.stopPropagation()}>
              <motion.div
                ...
                className='w-[90vw] max-w-[500px] aspect-square rounded-3xl relative cursor-default overflow-hidden shadow-2xl border border-white/10'
              >
                <img
                  src={items[index].url}
                  alt='single-image'
                ...
                />
                <article className='bg-white/90 dark:bg-brand-charcoal/90 backdrop-blur-xl absolute bottom-0 left-0 w-full p-8 border-t border-white/10'>
                  <motion.h1
                    initial={{ y: 20, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
                    exit={{ y: 20, opacity: 0 }}
                    transition={{ duration: 0.3, delay: 0.1 }}
                    className='text-2xl md:text-3xl font-serif text-brand-charcoal dark:text-white mb-3'
                  >
                    {items[index].title}
                  </motion.h1>
                  <motion.p
                    ...
                    className='text-sm md:text-base leading-relaxed text-brand-charcoal/70 dark:text-white/70'
                  >
